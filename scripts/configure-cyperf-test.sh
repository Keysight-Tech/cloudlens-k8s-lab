...
# ============================================================================
# Creates the "CloudLens Lab Traffic" test configuration on the CyPerf
# Controller via REST API. Configures DUT (Device Under Test) mode where
# traffic flows through the cyperf-proxy pod (round-robin to nginx pods)
...
#   ./configure-cyperf-test.sh 44.241.86.190                              # Explicit IP
#   ./configure-cyperf-test.sh 44.241.86.190 cloudlens-lab us-west-2 10.1.5.42
# ============================================================================
...
if [[ "$LICENSE_COUNT" -eq 0 ]]; then
    ...
    echo "  2. Login: admin / CyPerf&Keysight#1"
...
for s in sessions:
    if 'CloudLens Lab' in s.get('name','') or s.get('name','').startswith('SE Training'):
        print(s['id'])
        break
" 2>/dev/null)
...
log_info "Configuring DNS resolver on Client Network (AWS VPC DNS: 10.1.0.2)"
